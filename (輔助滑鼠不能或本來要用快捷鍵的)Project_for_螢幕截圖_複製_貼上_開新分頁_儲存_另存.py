...
delay_Save_page, delay_Save_As = 2,2  #用來讓跳前一頁和跳下一頁不會因為太敏感而連續執行太多次。讓它間隔2秒
...
def BEHAVIOR(frame, gesture_str, keypoint_pos, angle_list, bounding):
    ...
    ##當只有12伸出，朝左，則儲存檔案(每次需間隔delay_Save_page秒)
    if gesture_str == "12_left":
        ...
        if LATE_TIME_Save_page - TIME_Save_page > delay_Save_page or TIME_Save_page==0:  #讓每次單擊都間隔delay_Save_page秒
            gesture_str = gesture_str[:2] + "_Save_page"
            ...
            gesture_str = gesture_str[:2] + "_no_Save_page"

    ##當只有12伸出，朝右，則另存新檔(每次需間隔delay_Save_As秒)
    if gesture_str == "12_right":
        ...
        if LATE_TIME_Save_As - TIME_Save_As > delay_Save_As or TIME_Save_As==0:  #讓每次單擊都間隔delay_Save_As秒
            gesture_str = gesture_str[:2] + "_Save_As"
            ...
            gesture_str = gesture_str[:2] + "_no_Save_As"

    ## 當2345，則開新分頁
    if gesture_str == "2345":
        LATE_TIME_New_page = time.time()
        if LATE_TIME_New_page - TIME_New_page > delay_New_page or TIME_New_page==0:  #讓每次單擊都間隔delay_New_page秒
            gesture_str = gesture_str[:2] + "_New_page"
            TIME_New_page = time.time()

            #做出開新分頁的動作
            pyautogui.hotkey('ctrl', 'n')

        else:
            gesture_str = gesture_str[:2] + "_no_New_page"
    ...
    ##當只有234伸出，
    #       且23很近(34遠)，則執行複製(ctrl+c)  每次間隔1秒
    #       且34很近(23遠)，則執行貼上(ctrl+v)  每次間隔3秒
    if gesture_str == "234":
        ...
        if length_23<30 and length_34>30:  #do copy when 23近  34遠(23要疊在一起才有)
            cv2.circle(frame, (lineInfo_23[0], lineInfo_23[1]),15, (0, 136, 0), cv2.FILLED)
            LATE_TIME_copy = time.time()
            if LATE_TIME_copy - TIME_copy > delay_copy or TIME_copy==0:  #讓每次單擊都間隔delay_copy秒
                gesture_str = gesture_str + "_copy"
                TIME_copy = time.time()
                ...
                gesture_str = gesture_str + "_no_copy"

        elif length_34<30 and length_23>30:  #do paste when 34近  23遠(34要疊在一起才有)
            cv2.circle(frame, (lineInfo_34[0], lineInfo_34[1]),15, (80, 255, 0), cv2.FILLED)
            LATE_TIME_paste = time.time()
            if LATE_TIME_paste - TIME_paste > delay_paste or TIME_paste==0:  #讓每次單擊都間隔delay_paste秒
                gesture_str = gesture_str + "_paste"
                TIME_paste = time.time()

                #做出貼上的動作
                pyautogui.hotkey('ctrl', 'v')

            else:
                gesture_str = gesture_str + "_no_paste"
